...
const articleVideos = computed(() => videos.filter((v) => v.subject === route.params.slug).slice(0, 6))
...
watchEffect(() => {
  if (route.params.slug && !article.value) router.replace('/wiki/regulamentos')
})
</script>
...
<template>
  <section class="page-head">
    <div class="container">
      <p class="sec-label light">Wiki teórica</p>
      <h1>Material teórico das matérias</h1>
      <p class="head-lead">
        Resumo dos tópicos cobrados na prova teórica da ANAC, compilado de fontes públicas e
        citado em cada artigo. Use como guia de estudo junto às vídeo-aulas.
      </p>
    </div>
  </section>
  ...
  <section class="wiki-body">
    <div class="container wiki-layout">
      <nav class="wiki-nav" aria-label="Matérias">
        <p class="wiki-nav-label">Matérias</p>
        <router-link
          v-for="a in articles"
          :key="a.slug"
          :to="`/wiki/${a.slug}`"
      ...
      <article v-if="article" class="wiki-article">
        ...
        <section v-if="articleVideos.length" class="wiki-videos">
          <h3>Vídeo-aulas relacionadas</h3>
          <div class="wiki-video-grid">
            <VideoCard v-for="video in articleVideos" :key="video.id" :video="video" />
          </div>
        </section>

        <!-- Discreet sources footer -->
        <footer v-if="article.sources && article.sources.length" class="wiki-sources">
          Fontes:
          <template v-for="(src, i) in article.sources" :key="i">
            <a :href="src.url" target="_blank" rel="noopener noreferrer">{{ src.label }}</a
            ><span v-if="i < article.sources.length - 1">; </span>
          </template>
        </footer>
